...
def get_networks(var, expl_env):
    """
    Define Q networks and policy network
    """
    variant = var.copy()
    qf_kwargs = variant["qf_kwargs"].copy()
...
def get_path_collector(var, expl_env, eval_env, policy, eval_policy):
    """
...
def sac(var, resume_from=None, cpu=False):
    variant = var.copy()
    ...
    if resume_from is None:
        ...
        data = torch.load(resume_from, map_location=torch.device("cpu" if cpu else "cuda"))
