...
from nmp.launcher.curuculum import Curriculum
...
@click.option("-grid-size", "--grid-size", default=5, type=int)
@click.option("-max-epochs", "--max-epochs", default=1500, type=int)
@click.option("-epochs-per-curu", "--epochs-per-curu", default=100, type=int)
@click.option("-curu-obstacles-step", "--curu-obstacles-step", default=4, type=int)
@click.option("-success-to-next", "--success-to-next", default=0.5, type=float)
@click.option("-curu-dist-step", "--curu-dist-step", default=0.2, type=float)
@click.option(
    "-snap-mode",
    "--snapshot-mode",
    default="last",
...
@click.option("-resumetraining", "--resumetraining", default=None, type=str)
def main(
    env_name,
    exp_dir,
    seed,
    ...
    qf_lr = learning_rate
    env_kwargs = dict(grid_size=grid_size,
                      n_obstacles=0,
    ...
    curu = Curriculum(variant, grid_size=grid_size, success_to_next=success_to_next, max_epochs=max_epochs, epochs_per_curu=epochs_per_curu, curu_dist_step=curu_dist_step,
                      curu_obstacles_step=curu_obstacles_step, cpu=cpu, resume_from=resumetraining)
    print(f"Start CURU...")
    curu.train()
